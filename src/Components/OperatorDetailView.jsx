...
const OperatorDetailView = ({ operator, onBack, getSeverityColor, getCategoryColor }) => {
  ...
  const generateEnrollmentReviews = () => {
    const enrollmentTypes = ['New Enrollment', 'Update', 'Correction', 'Re-enrollment'];
  ...
  const handleReviewAction = (eidId, action) => {
    ...
    // Show notification
    const actionText = action === 'approved' ? 'Approved' : action === 'rejected' ? 'Rejected' : action === 'hold' ? 'On Hold' : 'Marked as Pending';
    console.log(`Enrollment ${eidId} ${actionText}`);
  };
      ...
      <div className="max-w-7xl mx-auto">
          ...
          <div className="p-6">
            ...
            <div className="flex gap-2 mt-6 border-t border-[#d2c5e7] border-opacity-60 pt-4 flex-wrap">
              {[
                ...
                { id: 'enrollmentReview', label: 'Enrollment Review', icon: FileText },
        ...
        <div className="p-6 space-y-6">
          ...
          {activeSection === 'enrollmentReview' && (
            <div className="space-y-6 animate-fade-in">
              {/* Header Card */}
              <div className="bg-gradient-to-r from-indigo-600 via-blue-600 to-purple-600 rounded-2xl shadow-2xl p-8 text-white">
                <div className="flex items-center justify-between">
                  <div>
                    <h2 className="text-4xl font-bold mb-2 flex items-center gap-3">
                      <FileText className="w-10 h-10" />
                      Enrollment Review Dashboard
                    </h2>
                    <p className="text-blue-100 text-lg">Review and manage operator enrollment records</p>
                  </div>
                  <div className="text-right">
                    <div className="text-sm text-blue-200 mb-1">Total Enrollments</div>
                    <div className="text-5xl font-bold">{enrollmentReviews.length}</div>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="bg-gradient-to-br from-green-50 to-green-100 rounded-xl p-6 border-2 border-green-200 shadow-lg hover:shadow-xl transition-all">
                  <div className="flex items-center justify-between">
                    <div>
                      <div className="text-sm font-semibold text-green-600 uppercase tracking-wide mb-1">Approved</div>
                      <div className="text-4xl font-bold text-green-700">
                        {enrollmentReviews.filter(r => r.status === 'approved').length}
                      </div>
                    </div>
                ...
                <div className="bg-gradient-to-br from-red-50 to-red-100 rounded-xl p-6 border-2 border-red-200 shadow-lg hover:shadow-xl transition-all">
                  <div className="flex items-center justify-between">
                    <div>
                      <div className="text-sm font-semibold text-red-600 uppercase tracking-wide mb-1">Rejected</div>
                      <div className="text-4xl font-bold text-red-700">
                        {enrollmentReviews.filter(r => r.status === 'rejected').length}
                      </div>
                    </div>
                ...
                <div className="bg-gradient-to-br from-orange-50 to-orange-100 rounded-xl p-6 border-2 border-orange-200 shadow-lg hover:shadow-xl transition-all">
                  <div className="flex items-center justify-between">
                    <div>
                      <div className="text-sm font-semibold text-orange-600 uppercase tracking-wide mb-1">On Hold</div>
                      <div className="text-4xl font-bold text-orange-700">
                        {enrollmentReviews.filter(r => r.status === 'hold').length}
                      </div>
                    </div>
                        ...
                        >
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="flex items-center justify-center gap-2">
                              <button
                                ...
                                className={`px-4 py-2 rounded-lg font-semibold text-xs transition-all duration-300 flex items-center gap-1.5 ${
                                  review.status === 'approved'
                                    ...
                                    : 'bg-green-100 text-green-700 hover:bg-green-500 hover:text-white hover:scale-105 hover:shadow-lg border border-green-300'
                                }`}
                                title="Approve"
                              >
                              ...
                              <button
                                ...
                                className={`px-4 py-2 rounded-lg font-semibold text-xs transition-all duration-300 flex items-center gap-1.5 ${
                                  review.status === 'rejected'
                                    ? 'bg-red-500 text-white shadow-lg cursor-default'
                                    : 'bg-red-100 text-red-700 hover:bg-red-500 hover:text-white hover:scale-105 hover:shadow-lg border border-red-300'
                                }`}
                                title="Reject"
                              >
                              ...
                              <button
                                onClick={() => handleReviewAction(review.id, 'hold')}
                                disabled={review.status === 'hold'}
                                className={`px-4 py-2 rounded-lg font-semibold text-xs transition-all duration-300 flex items-center gap-1.5 ${
                                  review.status === 'hold'
